...
| 34 | `learnUpload(type, records)` | type, records | `Promise<{asrMappingsAdded, ocrMappingsAdded}>` | 上传标注数据集，后端自动生成纠错映射 |
...
#### `POST /api/learn/upload` — 上传标注数据集（内部）

内部人员测试采集的原始文件 + 人工标注答案，上传后后端自动跑 ASR/OCR 对比生成纠错映射。
...
|------|------|------|
| `type` | `"asr" \| "ocr"` | 数据集类型 |
| `records` | `array` | 标注记录数组 |

**ASR 记录格式：**

| 字段 | 类型 | 说明 |
|------|------|------|
| `fileUrl` | `string` | 录音文件 URL（.pcm） |
| `correctText` | `string` | 人工标注的正确文本 |

**OCR 记录格式：**

| 字段 | 类型 | 说明 |
|------|------|------|
| `fileUrl` | `string` | 票据图片 URL |
| `correctCategory` | `string` | 人工标注的正确分类 |
| `correctAmount` | `string` | 人工标注的正确金额 |
| `correctNote` | `string` | 人工标注的备注（可选） |

**响应：** `{ "ok": true, "asrMappingsAdded": 3, "ocrMappingsAdded": 5 }`

| 字段 | 类型 | 说明 |
|------|------|------|
| `asrMappingsAdded` | `number` | 本次新增的 ASR 纠错映射条数 |
| `ocrMappingsAdded` | `number` | 本次新增的 OCR 商户映射条数 |

**后端处理流程：**
1. 遍历 records，每个 record 重新调用阿里云 ASR / OCR
2. ASR：拿阿里云返回文本和 `correctText` 做 diff，提取错词对
3. OCR：拿阿里云返回文本提取商户名，和 `correctCategory` / `correctNote` 配对
4. 新映射追加写入 `asr_corrections.json` / `ocr_merchants.json`
5. 返回新增映射数量
...
> 当前 OCR / ASR 的原始识别结果到记账字段的映射准确度有限，需建立一套可迭代的关键字映射表，后端从识别文本中抽取金额、分类、备注。

#### 一、ASR 语音映射

**输入**：用户口述的自由文本，如 `"午餐花了25块"`、`"打车15"`、`"收到工资八千"`

**输出**：`{ type: 'expense'|'income', amount: '25.00', category: '餐饮', note: '午餐' }`
...
| 模式 | 示例 | 输出 |
|------|------|------|
| `\d+(\.\d{1,2})?\s*[元块]` | `"25元"`、`"15.5块"` | `amount = 25` / `15.5` |
| `[元块]\s*\d+(\.\d{1,2})?` | `"花了25"`、`"消费八十"` | `amount = 25`（中文数字转阿拉伯） |
| 中文数字 | `"一百二"`、`"两千五"`、`"八千"` | `amount = 120` / `2500` / `8000` |

**中文数字映射：**
```
零=0  一=1  二/两=2  三=3  四=4  五=5  六=6  七=7  八=8  九=9  十=10
...
| 关键词前缀 | 判定 |
|-----------|------|
| `收到`、`收入`、`工资`、`报销`、`退款`、`进账`、`赚了` | `type = 'income'` |
| 其他（默认） | `type = 'expense'` |
...
| 分类 | 关键词 |
|------|--------|
| 餐饮 | 餐、饭、食、吃、外卖、美团、饿了么、食堂、早餐、午餐、晚饭、宵夜、火锅、烧烤、奶茶、咖啡、水果、菜 |
| 交通 | 打车、滴滴、地铁、公交、高铁、火车、机票、加油、油费、停车、过路费、出租车、骑行、共享单车 |
| 购物 | 买、购、淘宝、京东、拼多多、超市、商场、便利店、日用品、衣服、鞋、电器、数码 |
| 日用 | 水电、电费、水费、燃气、物业、房租、租金、房贷、网费、话费、宽带 |
| 医疗 | 药、医院、诊所、挂号、体检、看病、牙科、中药 |
| 教育 | 书、课、培训、学费、考试、报名、文具 |
| 娱乐 | 电影、唱歌、KTV、游戏、旅游、景点、门票、运动、健身 |
| 工资 | 工资、薪资、薪水、奖金、提成、分红、报销 |
| 通讯 | 话费、流量、宽带、网费 |

##### 4. 备注抽取

金额和分类匹配后，剩余文本作为 `note`。优先取商户名/商品名，不超过 20 字。

---

#### 二、OCR 凭证映射

**输入**：OCR 返回的文本行数组，如 `["某某餐饮有限公司", "合计：128.00", "2026-06-24"]`
...
| 模式 | 示例 |
|------|------|
| `\d{4}[-/年]\d{1,2}[-/月]\d{1,2}[日]?` | `2026-06-24`、`2026年6月24日` |
| `\d{4}\.\d{1,2}\.\d{1,2}` | `2026.06.24` |
...
| 票据常见词 | 映射分类 | 备注建议 |
|-----------|---------|---------|
| 餐饮、酒楼、饭店、餐厅、火锅、烧烤、茶餐厅 | 餐饮 | 取商户名 |
| 加油站、石化、石油 | 交通 | `加油` |
| 医院、诊所、药房、大药房 | 医疗 | 取商户名 |
| 超市、便利店、百货、商场 | 购物 | 取商户名 |
| 滴滴、出租车、地铁、铁路 | 交通 | 取出行方式 |
| 酒店、宾馆、民宿 | 娱乐 | `住宿` |
| 移动、联通、电信 | 通讯 | `话费` |

后端在 OCR 解析流程中的 `guessCategory()` 已实现初版，此表持续扩充。

---

#### 三、后端实现建议

1. **关键字表独立配置文件**（如 `category_keywords.json`），不硬编码在路由里，方便后续追加
2. **中文数字→阿拉伯数字** 工具函数，覆盖 `零`~`万`
3. **ASR / OCR 共用一个分类匹配函数**，输入文本串，输出 `{ type, amount, category, note, date }`
4. **匹配优先级**：OCR 的合计行 > 商户名关键词 > 文本行关键词
5. **无法匹配时**：`category` 返回空字符串，前端让用户手动选择（当前已支持）

---

## 八、人工标注纠错映射（结构化数据集学习）

> 内部人员大量测试 → 按字段拆解标注 → 整理成电子表格 → 上传后端 → 后端学习语句结构 + 纠错映射 → 识别时自动应用。

### 5.1 核心思路：把一句话拆成 7 个字段

记账语音本质上是**固定句式**的短输入，可以拆成结构化字段。比如"记一笔榴莲50元"：

```
记一笔  |  元  |  个人  |  榴莲  |  餐饮  |  50  |  支出
动词    量词   主体    项目词    分类    金额   收支
```

后端学习这 7 个字段后，识别时不再靠简单的关键词匹配，而是理解语句结构。

### 5.2 ASR 语音识别 — 7 字段拆解

| # | 字段 | 说明 | 示例 |
|---|------|------|------|
| 1 | **动词** | 触发记账的动作词 | `记一笔`、`买`、`花了`、`付`、`收了`、`还` |
| 2 | **量词** | 金额单位 | `元`、`块`、`毛`、`万` |
| 3 | **主体词** | 公司还是个人 | `公司`、`个人`、`我`（缺省=个人） |
| 4 | **项目词（备注）** | 买了什么 / 做了什么 | `榴莲`、`水饺`、`打车`、`话费` |
| 5 | **分类词** | 对应预设分类 | `餐饮`、`交通`、`购物`、`通讯`、`医疗`... |
| 6 | **金额词** | 金额数字 | `50`、`一百二`、`3块5` |
| 7 | **收支关键词** | 支出还是收入 | `支出`、`收入`、`花了`=支出、`收了`=收入 |

#### 5.2.1 弹性解析规则（语序任意、缺省智能填充）

ASR 返回的文本语序不固定，后端采用**关键词驱动**的 7 字段弹性解析，而非固定正则。

**核心原则：**

| 规则 | 说明 |
|------|------|
| **金额必须** | 一句话至少包含一个数字，否则无法记账 |
| **语序任意** | "咖啡18" / "18咖啡" / "18元咖啡" 等价 |
| **缺省填充** | 未识别到的字段自动填默认值 |
| **无意义词忽略** | "我妈"、"了个"、"给我"、"一下"、"去了" 等跳过 |
| **时间词自动** | "今天/昨天/前天" → 自动匹配日期，不参与训练 |
...
```
正则: \d+(\.\d{1,2})?  或  中文数字（一百二 → 120）
提取后从原文中移除，不参与后续关键词匹配
```
...
```
收到, 收入, 赚了, 捡了, 发了, 转入, 报销, 退款, 到账, 转账, 酬劳, 补贴, 津贴
```
...
```
公司: 公司, 垫付, 应付, 报销(公司场景), 办公
个人: 个人, 我, 我的（缺省，无需显式声明）
```

除非明确出现公司侧关键词，否则一律 `个人`。

##### 4. 分类词 → 预设分类映射

| 关键词 | 分类 |
|--------|------|
| 咖啡, 奶茶, 柠檬茶, 可乐, 饮料, 牛奶, 豆浆 | 饮品 |
| 午餐, 晚餐, 早餐, 外卖, 吃饭, 火锅, 烧烤, 米粉, 面条, 面, 饺子, 盒饭, 快餐, 麻辣烫 | 餐饮 |
| 打车, 滴滴, 出租, 地铁, 公交, 加油, 高铁, 火车票, 机票, 停车 | 交通 |
| 水果, 零食, 买菜, 菜, 西瓜, 榴莲, 水果篮, 超市 | 食品 |
| 话费, 充值, 流量 | 通讯 |
| 感冒药, 药品, 药, 医院, 诊所 | 医疗 |
| 房租, 租房, 房贷, 物业 | 住房 |
| 购物券, 采购, 淘宝, 京东, 拼多多 | 购物 |
...
分类词命中后从原文移除，剩余文本作为备注。

##### 5. 项目词（备注）

提取完金额、收支、主体、分类后，**剩余的实义词**作为备注。
若分类未命中，备注本身可尝试直接作为分类名（如"咖啡"→分类=饮品, 备注=咖啡）。

##### 6. 动词（缺省不展示）

```
记一笔, 买, 花了, 付, 充, 发了, 收了, 还, 报销, 给
```
...
输入文本
  ...
  → 匹配主体关键词 → 定个人/公司
  → 匹配分类关键词 → 定分类
  → 剩余词清洗（去无意义词+动词+量词）→ 备注
  → 缺省填充（个人+支出+元+今天）
  → 组装返回 { amount, category, note, direction, scope, date }
```
...
| 输入 | 解析结果 |
|------|---------|
| `咖啡18` | 金额=18, 分类=饮品, 备注=咖啡, 个人, 支出 |
| `18咖啡` | 同上（倒装） |
| `捡了1800今天` | 金额=1800, 收支=收入(捡了), 分类=工资?, 备注=捡了1800, 日期=今天 |
| `我妈发了个红包给我` | 金额=?(缺→追问), 收支=收入(发了), 分类=人情(红包), 主体=个人 |
| `公司垫付打车30` | 金额=30, 分类=交通(打车), 主体=公司(垫付), 收支=支出(垫付=公司先出) |
| `报销差旅费320` | 金额=320, 分类=交通(差旅费), 主体=公司?(报销), 收支=收入(报销=钱进来) |

### 5.3 电子表格格式

内部人员用 Excel / CSV 填写，一行一条训练样本。**只需填原始文本 + 7 字段，正确文本由后端自动拼装。**

```
| 原始识别文本       | 动词   | 量词 | 主体 | 项目词 | 分类 | 金额 | 收支 |
|-------------------|--------|------|------|--------|------|------|------|
| 记一笔流连50元      | 记一笔 | 元   | 个人 | 榴莲   | 餐饮 | 50   | 支出 |
| 公司买水角30块      | 买     | 块   | 公司 | 水饺   | 餐饮 | 30   | 支出 |
| 交通会15元         | -      | 元   | 个人 | 交通费 | 交通 | 15   | 支出 |
| 充话费100块        | 充     | 块   | 个人 | 话费   | 通讯 | 100  | 支出 |
| 发了工资5000元     | 发了   | 元   | 个人 | 工资   | 收入 | 5000 | 收入 |
| 公司报销打车费30元  | 报销   | 元   | 公司 | 打车   | 交通 | 30   | 收入 |
| 买了两斤水果蓝85块  | 买了   | 块   | 个人 | 水果篮 | 购物 | 85   | 支出 |
```

**填写规则：**
- **原始识别文本**：阿里云 ASR 实际返回的内容（含错误）
- **7 字段**：以**人工听到的正确内容**为准填写（不是 ASR 返回的错词）
- 动词如果不是显式出现的词，填 `-`
- 主体不明确时默认为 `个人`
- **正确文本由后端自动拼装**：`动词 + 项目词 + 金额 + 量词` → 如 `记一笔榴莲50元`

### 5.4 后端学习逻辑

后端收到 CSV 后，先自动拼装正确文本，再学习三层知识：

**步骤 0：自动拼装正确文本**

```
7 字段 → 拼装规则: 动词 + 项目词 + 金额 + 量词

示例: 动词=记一笔, 项目词=榴莲, 金额=50, 量词=元 → "记一笔榴莲50元"
      动词=-, 项目词=交通费, 金额=15, 量词=元 → "交通费15元"
```

**步骤 1-3：三层学习**

#### 第一层：错词纠错（原始文本 → 自动拼装的正确文本）

逐行对比「原始识别文本」和「自动拼装的正确文本」，用 diff 算法提取错词对：

```
原始: "记一笔流连50元"  → 正确: "记一笔榴莲50元" → 映射: "流连"→"榴莲"
原始: "公司买水角30块"  → 正确: "公司买水饺30块" → 映射: "水角"→"水饺"
原始: "交通会15元"      → 正确: "交通费15元"     → 映射: "交通会"→"交通费"
```

写入 `asr_corrections` 表（HashMap），识别时 O(1) 替换。

#### 第二层：项目词 → 分类映射

从「项目词」和「分类」列学习物品到分类的对应关系：

```
"榴莲"→"餐饮"、"水饺"→"餐饮"、"交通费"→"交通"、"话费"→"通讯"、"水果篮"→"购物"
```

写入 `item_category_map` 表，识别时从备注反推分类。
...
| 学到的规律 | 来源 |
|-----------|------|
| `记一笔` + 物品 + 金额 + 量词 → 支出 | 动词驱动 |
| `买了` + 物品 + 金额 + 量词 → 支出 | 动词驱动 |
| `发了` + 物品 + 金额 + 量词 → 收入 | 动词 = 收入信号 |
| `公司` + 任意 → scope = company | 主体驱动 |
| 无主体 → scope = personal | 缺省规则 |
| `报销` → 收入 | 动词驱动 |

后端用这些规律在识别时做**结构化推断**，而不只是关键词匹配。

### 5.5 OCR 商户分类映射（保持不变）

OCR 票据识别继续沿用商户关键词 → 分类映射，详见 5.8 节。

### 5.6 数据集上传接口（ASR 结构化版）
  ...
  "records": [
    {
      ...
      "amount": "50",
      "direction": "支出",
      "correctText": "记一笔榴莲50元"
    }
  ]
}
```

**方式二：CSV 文件上传（推荐，直接传电子表格）**

```
POST /api/learn/upload-csv
Authorization: Bearer <token>
...
CSV 列顺序：`原始识别文本,动词,量词,主体,项目词,分类,金额,收支,正确文本`
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `totalRows` | `number` | 上传的训练样本总行数 |
| `correctionsAdded` | `number` | 新增错词纠错映射数（"流连"→"榴莲"） |
| `itemCategoryMappingsAdded` | `number` | 新增项目词→分类映射数 |
| `patternsLearned` | `number` | 新学到的句式结构模式数 |
...
逐行解析 7 字段
       ↓
┌──────────────────────────────────────┐
│ 第一层：diff(rawText, correctText)    │
│   → 提取错词对 → 写入 asr_corrections │
├──────────────────────────────────────┤
│ 第二层：(item, category) 配对          │
│   → 写入 item_category_map            │
├──────────────────────────────────────┤
│ 第三层：(verb, subject, direction) 组合 │
│   → 提取句式模式 → 写入 asr_patterns   │
└──────────────────────────────────────┘
       ↓
返回学习结果统计
```
...
| 表 | 内容 | 示例 |
|----|------|------|
| `asr_corrections` | 错词 → 正确词 | `{ "流连": "榴莲", "水角": "水饺" }` |
| `item_category_map` | 项目词 → 分类 | `{ "榴莲": "餐饮", "话费": "通讯" }` |
| `asr_patterns` | 句式模式 | `{ "verb": "买了", "direction": "支出", "defaultSubject": "个人" }` |
| `ocr_merchants` | 商户关键词 → 分类+备注 | `{ "京东": { "category": "购物", "note": "京东" } }` |

全部加载到内存，识别时直接查 HashMap，性能零损耗。
...
ASR:
  录音 → 阿里云 NLS → 原始文本
    → 第一层：查 asr_corrections 替换错词
    → 第二层：查 asr_patterns 匹配句式，确定 动词/主体/收支
    → 第三层：查 item_category_map 匹配项目词 → 分类
    → 第四层：正则提取金额
    → 组装结果返回前端

OCR:
  图片 → 阿里云 OCR → 原始文本
    → 提取商户名 → 查 ocr_merchants 匹配分类
    → 正则提取金额/日期
    → 返回前端
```
...
> "今天早上吃一碗米粉20元，去买了水果花了32元，然后打了滴滴12元，并且发人情红包200元，还去药店买了一下感冒药12元，对了还有工资5000元到账了"

系统需要自动**断句**并**逐条识别**，返回多笔结果。
...
| 优先级 | 断句方式 | 示例 |
|--------|---------|------|
| 1 | **连接词** | `然后`、`并且`、`还`、`还有`、`对了`、`另外`、`接着`、`再` |
| 2 | **标点符号** | `，`、`。`、`；`、`,`、`.` |
| 3 | **金额+量词边界** | `xx元/块` 后面紧接新动词时切开 |
...
原始长文本
    ↓
正则切分：按连接词 + 标点拆成 N 个短句
    ↓
逐句过 ASR 纠错 pipeline（同短句流程 5.9）
    ↓
组装返回: [{ amount, category, note, direction, scope }, ...]
```

#### 断句示例

```
输入: "今天早上吃一碗米粉20元，去买了水果花了32元，然后打了滴滴12元，
       并且发人情红包200元，还去药店买了一下感冒药12元，对了还有工资5000元到账了"

断句结果:
  分句1: "吃一碗米粉20元"       → 项目词=米粉, 分类=餐饮, 金额=20, 支出
  分句2: "买了水果花了32元"      → 项目词=水果, 分类=购物, 金额=32, 支出
  分句3: "打了滴滴12元"          → 项目词=滴滴, 分类=交通, 金额=12, 支出
  分句4: "发人情红包200元"       → 项目词=红包, 分类=人情, 金额=200, 支出
  分句5: "买了感冒药12元"        → 项目词=感冒药, 分类=医疗, 金额=12, 支出
  分句6: "工资5000元到账"        → 项目词=工资, 分类=收入, 金额=5000, 收入

前端弹窗展示 6 笔结果，用户确认后批量写入。
```

#### ASR 接口响应扩展

短句模式（现行为 `POST /api/asr/recognize`）：
...
```json
{
  ...
  "text": "记一笔榴莲50元",
  "results": [
    { "amount": "50", "category": "餐饮", "note": "榴莲", "direction": "支出", "scope": "personal" }
  ]
}
```

长文本模式 — `results` 数组返回多笔：

```json
{
  "ok": true,
  "text": "吃一碗米粉20元，买了水果花了32元，然后打了滴滴12元...",
  "results": [
    { "amount": "20", "category": "餐饮", "note": "米粉", "direction": "支出", "scope": "personal" },
    { "amount": "32", "category": "购物", "note": "水果", "direction": "支出", "scope": "personal" },
    { "amount": "12", "category": "交通", "note": "滴滴", "direction": "支出", "scope": "personal" },
    { "amount": "200", "category": "人情", "note": "红包", "direction": "支出", "scope": "personal" },
    { "amount": "12", "category": "医疗", "note": "感冒药", "direction": "支出", "scope": "personal" },
    { "amount": "5000", "category": "收入", "note": "工资", "direction": "收入", "scope": "personal" }
  ]
}
```

前后端约定：`results` 长度 ≤ 1 为短句模式，> 1 为长文本多笔模式。前端根据长度决定展示单条还是批量确认列表。

#### 长文本训练数据集

长文本训练用独立的 CSV 模板，格式为「原始长文本 + 逐句拆解」：

| 列 | 说明 |
|----|------|
| `原始长文本` | 阿里云 ASR 返回的完整长文本 |
| `断句序号` | 该长文本内的第几句（1, 2, 3...） |
| `原始分句` | 断句后该句的 ASR 原始文本 |
| 7 字段 | 动词、量词、主体、项目词、分类、金额、收支 |

正确分句由后端自动拼装（同短句规则：`动词 + 项目词 + 金额 + 量词`），无需人工填写。

同一段长文本占多行，按断句序号分组。

**上传接口**：同样走 `POST /api/learn/upload-csv`，`type=asr_longtext`。

后端学习时额外学两样东西：
1. **断句边界库**：从长文本中提取连接词/标点模式，积累到 `split_patterns`
2. **逐句纠错**：每个分句走标准 7 字段学习流程（同短句）

### 5.11 内部数据采集页面（前端）

> 可选，仅内部人员使用，不打包进正式版。

- **ASR 模式**：批量上传 CSV 文件到 `POST /api/learn/upload-csv`
- **OCR 模式**：上传票据图片 + 人工标注分类/金额/备注
- 此页面可放在调试入口或独立内部工具中，普通用户不可见。

### 5.12 AI 财务分析（后期接入）

> **核心原则：用户信用授权 + 数据最小化 + 本地优先。**

#### 风险点

- 财务分析需要读取用户全量账单数据（跨月、跨年、跨账本）
- 涉及用户隐私底线，处理不当极易引发信任危机
- 国内算法备案合规要求（若调用生成式 AI）

#### 分期策略

| 阶段 | 功能 | 数据范围 | 门槛 |
|------|------|---------|------|
| **基础版** | 月汇总 / 分类占比 / 趋势图 | 本地统计，不出设备 | 无门槛，所有用户 |
| **高级版** | AI 消费建议 / 预算预警 / 异常检测 | 脱敏聚合数据上送 | 实名认证 + 连续记账 30 天 |
| **专业版** | AI 完整财务报告 / 税务建议 | 需明确授权 + 协议勾选 | 信用分达标 + 零投诉记录 |

#### 数据安全设计

1. **本地优先**：基础统计（环比、同比、分类占比）纯客户端计算，不传后端
2. **脱敏上送**：若需后端 AI 分析，只传聚合统计值（如 `{ "餐饮": { "本月": 3200, "上月": 2800 } }`），不传原始账单明细
3. **可撤销**：用户可随时关闭 AI 分析并清除已上传数据
4. **隔离存储**：AI 分析结果与账单原始数据分库存储

#### 信用门槛

```
实名认证 → 连续记账 N 天 → 零投诉记录 → 解锁 AI 分析
```
...

{
  ...
  "metrics": ["trend", "category_breakdown", "budget_alert"]
}

响应：聚合后的分析结果，不含原始账单行。
```

> 此功能暂不实现，仅为后期规划占位。当前聚焦关键词驱动记账核心体验。
...
### 7.8 ❌ 待修复 — OCR 凭证图片持久化：记账需长期持有凭证图 + 保存 OCR 识别结果

**问题**：OCR 识别记账流程中，凭证图片和 OCR 识别结果无法持久化保存。

**当前流程**：

```
拍照/选图 → uploadVoucher(photo) 上传到后端 → 返回 URL
       ↓
ocrParse(url) 用 URL 调 OCR 识别 → 返回 { amount, category, note }
       ↓
bookForm 填入 OCR 结果 + bookPhoto = 本地临时路径
       ↓
用户点保存 → onBookSave()
       ↓
voucher = saveFileSync(bookPhoto)  ← 存的是本地临时路径!!
       ↓
addItem({ voucher: 'wxfile://tmp_xxx' }) → POST /items
       ↓
后端收到 voucher = 本地路径，无法访问 → 可能丢弃整条记录
```

**根因**：`uploadVoucher` 返回的云端 URL 在 OCR 流程中被丢弃了，保存时 `bookPhoto` 仍是本地临时路径，传给后端的 `voucher` 字段是一个后端无法访问的 `wxfile://` 路径。

**影响**：
- OCR 识别后保存的账单可能在**后端同步时丢失**（其他记账方式正常）
- 凭证图片仅存本地临时目录，卸载/清理后丢失
- 跨设备登录后看不到凭证图片

**期望后端改动**：

1. **`POST /items` 兼容 `voucher` 字段**：
   - 若 `voucher` 值已是完整 HTTP URL（如 `http://host/voucher/4/2026/06/xxx.jpg`），直接存储
   - 若 `voucher` 是本地路径或为空，跳过（不因此拒绝整条记录）

2. **凭证图片长期持有**：
   - `POST /api/upload` 上传后应立即将文件移至持久化目录（`voucher/{safeId}/{year}/{month}/`）
   - 不因 OCR 识别完成就删除图片
   - 账单保存时关联已上传的凭证 URL

**前端需配合**（待后端就绪后修改）：

在 `_startScanRecognize` 中保存 `uploadVoucher` 返回的 URL，`onBookSave` 时将此 URL 作为 `voucher` 字段传给 `addItem`，而非本地临时路径：

```js
// _startScanRecognize 中
var uploadedUrl = ''
api.uploadVoucher(photo).then((url) => {
  uploadedUrl = url           // 保存云端 URL
  this._uploadedVoucherUrl = url
...
// onBookSave 中
voucher: this._uploadedVoucherUrl || ''  // 用云端 URL
```
